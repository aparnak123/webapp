...
# add S3 credentail to ES container for snapshot repo
run_as_other_user_if_needed echo "$AWS_ACCESS_KEY_ID" | elasticsearch-keystore add --stdin --force --verbose 's3.client.default.access_key'
run_as_other_user_if_needed echo "$AWS_SECRET_KEY" | elasticsearch-keystore add --stdin --force --verbose 's3.client.default.secret_key'
